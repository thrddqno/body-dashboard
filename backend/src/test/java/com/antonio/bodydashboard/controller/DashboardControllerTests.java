...
class DashboardControllerTests {
	...
	void returnsCompleteDashboardSnapshot() throws Exception {
		...
		mockMvc.perform(get("/api/dashboard"))
				...
				.andExpect(jsonPath("$.body.activeTargetKg").value(104.0))
				.andExpect(jsonPath("$.body.weightRemainingKg").value(7.75))
				.andExpect(jsonPath("$.body.goal.stage1TargetKg").value(104.0))
				.andExpect(jsonPath("$.body.goal.stage2MinKg").value(100.0))
				.andExpect(jsonPath("$.body.goal.stage2MaxKg").value(101.0))
				.andExpect(jsonPath("$.body.goal.calorieTargetKcal").value(2500))
				.andExpect(jsonPath("$.body.goal.estimatedMaintenanceMinKcal").value(3000))
				.andExpect(jsonPath("$.body.goal.estimatedMaintenanceMaxKcal").value(3300))
