...
export function DashboardSidePanels({ dashboard, analytics, latestAnalysis, hasAnalysis }: DashboardSidePanelsProps) {
    ...
    <aside className="space-y-5" aria-label="Progression and targets">
      <section className="rounded-[8px] bg-[var(--green)] p-8 text-white">
        <div className="flex items-center justify-between gap-4">
          ...
          <span className={`inline-flex rounded-[8px] px-2.5 py-1 text-[10px] font-black uppercase ${verdictStyles[decision.verdict]}`}>
        ...
        {decision.factors.length > 0 ? (
          <ul className="mt-4 space-y-2 border-b border-[var(--green-line)] pb-4">
            {decision.factors.map((factor) => (
              <li key={factor} className="flex gap-2 text-xs leading-5 text-[var(--green-copy)]">
        ...
        <div className="mt-4">
          <p className="text-[10px] font-black uppercase text-[var(--lime)]">What's next</p>
          {recommendations.length > 0 ? (
            <ul className="mt-2 space-y-2">
              {recommendations.slice(0, 2).map((recommendation) => (
                <li key={recommendation} className="text-[13px] leading-[1.65] text-[var(--green-copy)]">
                  {recommendation}
                </li>
              ))}
            </ul>
          ...
          <Link to="/analysis" className="button-secondary mt-4 inline-flex">Open full analysis</Link>
        </div>
      </section>
